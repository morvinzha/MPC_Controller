...
dotx[4:6]	=	([0;0;-gravity]+(1/mess)*dyn_k*norm(x[14:17])^2*genRot([0;0;0])-1/mess*dragA*x[4:6])*x[13]
dotx[7:9]	=	Rot(x[7:9])*x[10:12]*x[13]	#roll pitch yaw
...
function genRot(x)
return [cos(x[3])*sin(x[2])*cos(x[1])+sin(x[3])*sin(x[1]);sin(x[3])*sin(x[2])*cos(x[1])-cos(x[3])*sin(x[1]);cos(x[2])*cos(x[1])]
end
...
tao[1]=Ldis*dyn_k*m1
tao[2]=Ldis*dyn_k*m2
tao[3]=tao_b*(w[1]^2+w[3]^2-w[4]^2-w[2]^2)
return tao
end

function Rot(x)
	return [1 sin(x[1])*tan(x[2]) cos(x[1])*tan(x[2]);0 cos(x[1]) -sin(x[1]);0 sin(x[1])/cos(x[2]) cos(x[1])/cos(x[2])]
end
